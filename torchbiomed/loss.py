...
#       print(input)
        ...
        result_sum = torch.sum(result*result)
        ...
        out = 1 - torch.FloatTensor(1).fill_(2*IoU)
    ...
    def backward(self, grad_output):
        ...
        grad_input = torch.cuda.FloatTensor(input.size())
        grad_input[:, 0]= torch.mul(dDice, grad_output[0])
        grad_input[:, 1]= torch.mul(dDice, -grad_output[0])
        # torch.cat((torch.mul(dDice, grad_output[0]),
